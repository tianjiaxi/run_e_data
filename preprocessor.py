...
class EntityTypes(object):
    ...
    def build_types_embedding(
        ...
        if types_mode.lower() == "cls":
            ...
            # weight is replaced with torch.rand
        self.types_embedding.requires_grad = False
        # type embedding does not need gradient, if update just by update_embedding flag.
...
class Corpus(object):
    def __init__(
        ...
        # if we want to train multiple bert span detector, just use reset-meta and re-shuffle it.
    ...
    def _convert_example_to_feature_(
        ...
        for words, labels in zip(example.words, example.labels):
            ...
            # here the token_sum means how many tokens the word has
