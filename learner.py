...
class Learner(nn.Module):
    ...
    def __init__(
        ...
        # load model
        if model_dir != "":
            # here we set that when model_dir is not empty, we want to evaluate and eval_mode is not two stage
            if self.eval_mode != "two-stage":
                self.load_model(self.eval_mode)
        else:
            logger.info("********** Loading pre-trained model **********")
            # normally we should set a cache_dir here
            ...
            # if we want to fit extra data trained model with new training set, please add them here.
    ...
    def forward_supervise(self, batch_query, batch_support, progress, inner_steps):
        # here batch query and batch support length are all defined by batch size
        span_losses, type_losses = [], []
        task_num = len(batch_query)
        # for each task in the batch, they all use it to model once
    ...
    def forward_meta(self, batch_query, batch_support, progress, inner_steps):
        ...
        # I always think meta method add batch??
    ...
    def evaluate_meta_(
        ...
        if self.eval_mode == "two-stage":
            ...
            # 'span' parameter could set train mode as span
        names = self.get_names()
        params = self.get_params()
        weights = deepcopy(params)
        ...
        preds = None
        t_tmp = time.time()
        # predes is for continuous evaluate
        ...
        for item_id in range(corpus.n_total):
            ...
            # when we use re-train extra data, we should also set eval()
            self.model.eval()
            with torch.no_grad():
                ...
                if self.model.train_mode != "span":
                    # we have type and add two directions
                    if self.model.train_mode == "add" and self.eval_mode == "add":
