...
def get_data_path(agrs, train_mode: str):
    # here the dataset means if we use fewnerd
...
def train_meta(args):
    ...
    # since valid and test need to evaluate so they need viterbi parameters
    ...
    # here the args.dataset means FewNerd or domain
    if args.debug:
    ...
    for step in range(args.max_meta_steps):
        ...
        if step % args.eval_every_meta_steps == 0 and step > protect_step:
            ...
            if (
                ...
                # remember here to check the result_dir parameters.
...
def test(args, learner, corpus, types: str):
    if corpus.viterbi != "none":
        ...
        # acturally the following types do not mean much
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--add_extra",
        action="store_true",
        help="if true, will load the extra data",
    )
    parser.add_argument(
        "--test_only",
        action="store_true",
    ...
    if args.test_only:
        ...
        top_dir = "models-{}-{}-{}".format(args.N, args.K, args.mode)
        # here if we train a model, the model will be saved in this address.
        ...
        # here, if we change the batch size, the name should also be changed
